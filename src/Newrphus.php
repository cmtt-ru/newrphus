...
class Newrphus
{
    ...
    {
        try {
            $this->floodProtect(md5($misprintData['text']));

            $urlInfo = null;
            $userInfo = null;

            if (isset($misprintData['url'])) {
                $urlInfo = ['title' => 'URL', 'value' => $misprintData['url'], 'short' => true];

                if (is_callable($this->urlAnalysis)) {
                    $urlInfo = call_user_func($this->urlAnalysis, $misprintData['url']);
                }
            }

            if (isset($misprintData['userId']) && $misprintData['userId'] > 0) {
                $userInfo = ['title' => 'User ID', 'value' => $misprintData['userId'], 'short' => true];

                if (is_callable($this->userIdAnalysis)) {
                    $userInfo = call_user_func($this->userIdAnalysis, $misprintData['userId']);
                }
            }
